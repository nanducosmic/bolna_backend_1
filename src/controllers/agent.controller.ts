...
export const saveAgent = async (req: any, res: Response) => {
  try {
    const { name, prompt, bolnaAgentId, bolna_rag_id, purchasedPhoneNumber } = req.body;
    ...
    if (agent) {
      ...
      if (purchasedPhoneNumber) agent.purchasedPhoneNumber = purchasedPhoneNumber; // Save purchased phone number
      await agent.save();
    } else {
      agent = await Agent.create({
        ...
        bolnaAgentId,
        bolna_rag_id,
        purchasedPhoneNumber, // Save purchased phone number
        status: "active",
        type: "outbound"
      });
    }
